...
require 'csv'
  def show
   ...
   career_development = Sprint.joins(:tasks).where(tasks: { category: "career development" }).group_by_month(:sprint_date, range: (time_ago)..Time.now).unscope(:order).sum(:value)
    ...
    health.each do |row|
      csv << "health,#{row[1]},#{csv.each_line.count - 1},#{row[0]}\n"
    end

    vacation.each do |row|
      vacation_list << "vaction,#{row[1]},#{vacation_list.each_line.count},#{row[0]}\n"
    end

    personal.each do |row|
      personal_list << "personal,#{row[1]},#{personal_list.each_line.count},#{row[0]}\n"
    end

    gear_lift.each do |row|
      gear_lift_list << "gear_lift,#{row[1]},#{gear_lift_list.each_line.count},#{row[0]}\n"
    end

    snowmass.each do |row|
      snowmass_list << "snowmass,#{row[1]},#{snowmass_list.each_line.count},#{row[0]}\n"
    end

    fcfs.each do |row|
      fcfs_list << "fcfs,#{row[1]},#{fcfs_list.each_line.count},#{row[0]}\n"
    end

    one_dataset.each do |row|
      one_dataset_list << "one_dataset,#{row[1]},#{one_dataset_list.each_line.count},#{row[0]}\n"
    end

    board.each do |row|
      board_list << "board,#{row[1]},#{board_list.each_line.count},#{row[0]}\n"
    end

    career_development.each do |row|
      career_development_list << "career_development,#{row[1]},#{career_development_list.each_line.count},#{row[0]}\n"
    end
